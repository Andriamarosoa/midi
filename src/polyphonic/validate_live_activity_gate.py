...
def _replay_decoder(
    ...
    *,
    ...
    for trace_index in range(frames):
        ...
        onset_hop_index = None
        if use_audio_onset:
            if "decoder_audio_onset_hop_index" in debug:
                stored_index = int(
                    debug["decoder_audio_onset_hop_index"][trace_index]
                )
                onset_hop_index = stored_index if stored_index >= 0 else None
            elif "audio_onset" in debug and bool(
                debug["audio_onset"][trace_index]
            ):
                onset_hop_index = model_frame_index
        emitted = decoder.step(
            ...
            audio_hop_index=model_frame_index,
            audio_onset=(
                onset_hop_index is not None if use_audio_onset else None
            ),
            audio_onset_hop_index=onset_hop_index,
        )
