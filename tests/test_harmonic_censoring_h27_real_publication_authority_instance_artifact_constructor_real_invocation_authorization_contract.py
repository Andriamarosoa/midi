...
class TestContract(unittest.TestCase):
 ...
 def test_order_guards_authorization_state_and_edges(self):
  expected=['verify_exact_git_head_and_clean_worktree','rehash_all_one_hundred_four_predecessor_identities','validate_exact_native_runtime_types_before_magic_methods','validate_canonical_top_level_and_nested_orders_and_values','validate_strict_timestamp_nonce_issuer_destination_and_id_derivation','open_persistent_identity_nonce_registry','atomically_reserve_identity_and_nonce_terminally','invoke_constructor_once','verify_canonical_bytes_match_reserved_identity','first_destination_observation_probes_absence','create_exclusive_without_overwrite','terminal_success_or_terminal_post_reservation_failure_without_retry'];self.assertEqual(self.c['future_fail_closed_order'],expected);self.assertTrue(all(self.c['preserved_constructor_guards'].values()))
