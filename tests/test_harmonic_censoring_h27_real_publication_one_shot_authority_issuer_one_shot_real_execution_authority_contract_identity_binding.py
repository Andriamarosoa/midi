...
BINDING = ROOT / "configs/harmonic_censoring_h27_real_publication_one_shot_authority_issuer_one_shot_real_execution_authority_contract_identity_binding.json"
SEAL = ROOT / "configs/harmonic_censoring_h27_real_publication_one_shot_authority_issuer_one_shot_real_execution_authority_contract_identity_binding_external_seal.json"
...
def _eighty_eight(contract: dict) -> list[dict]:
    roots = contract["reviewed_and_sealed_real_issuance_authorization_chain"]
    authorization_binding = json.loads((ROOT / roots[2]["path"]).read_bytes())
    return [*roots, *_eighty_four(authorization_binding)]


class H27OneShotRealExecutionAuthorityBindingTests(unittest.TestCase):
    @classmethod
    ...
    def test_canonical_binding_seal_and_ninety_exact_identities(self) -> None:
        ...
        entries = [self.binding["reviewed_contract"], self.binding["contract_external_seal"], *_eighty_eight(contract)]
        self.assertEqual(len(entries), 90)
        self.assertEqual(len({item["path"] for item in entries}), 90)
        for item in entries:
            _check(self, item)

    def test_semantics_order_graph_and_closed_state(self) -> None:
        contract = json.loads((ROOT / self.binding["reviewed_contract"]["path"]).read_bytes())
        self.assertEqual(self.binding["preserved_normative_order"], contract["future_normative_order"])
        self.assertEqual(self.binding["preserved_authority_semantics"]["single_use_only"], contract["future_authority_definition"]["single_use_only"])
        for key in ("success_terminal", "post_consumption_failure_terminal", "retry_after_consumption_forbidden"):
            self.assertEqual(self.binding["preserved_authority_semantics"][key], contract["future_consumption_rules"][key])
        ...
        allowed = {
            "one_shot_real_execution_authority_contract_exists",
            "one_shot_real_execution_authority_contract_externally_reviewed",
            "one_shot_real_execution_authority_contract_externally_sealed",
            "one_shot_real_execution_authority_contract_identity_binding_exists",
        }
    ...
    def test_no_predecessor_references_new_binding_or_seal(self) -> None:
        contract = json.loads((ROOT / self.binding["reviewed_contract"]["path"]).read_bytes())
        predecessors = [self.binding["reviewed_contract"], self.binding["contract_external_seal"], *_eighty_eight(contract)]
