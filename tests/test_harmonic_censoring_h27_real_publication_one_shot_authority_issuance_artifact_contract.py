...
class TestContract(unittest.TestCase):
 ...
 def test_closed_schema_and_adversarial_values(self):
  ...
  f=s['fields'];source=json.loads((ROOT/f['issuer_id']['attested_source_path']).read_bytes());issuer=source['fixed_paths_and_counts']['issuer_identity'];self.assertEqual(issuer,'h27-execution-codex-mac-primary');self.assertEqual(f['issuer_id']['exact_value'],issuer);nonce='ab'*32;dest=f['destination_path']['exact_value'];pm=f['predecessor_manifest_sha256']['exact_value'];digest=hashlib.sha256((issuer+'\n'+nonce+'\n'+dest+'\n'+pm).encode()).hexdigest()
  ...
  bad=[{**good,'extra':1},{**good,'issuer_id':'h26-execution-codex-mac-primary'},{**good,'issuer_id':'other'},{**good,'invocation_nonce':'A'*64},{**good,'issued_at_utc':'2026-02-30T12:00:00Z'},{**good,'single_use':False},{**good,'predecessor_identity_count':True},{**good,'predecessor_manifest_sha256':'0'*64},{**good,'artifact_id':'h27-authority-'+'0'*64}]
  def valid(x):
