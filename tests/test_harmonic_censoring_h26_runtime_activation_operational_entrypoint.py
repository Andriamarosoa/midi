...
class OperationalEntrypointTests(unittest.TestCase):
    ...
    def test_exact_values_and_canonical_bytes_precede_adapter(self):
        ...
        with mock.patch.object(runner, "_require_execution_boundary", return_value=capability), mock.patch.object(runner, "_H26BoundaryGatedPosixIssuanceFilesystemAdapter") as adapter_type, mock.patch.object(runner, "publish_prevalidated_activation_with_adapter", return_value=receipt) as publish:
            result = runner.issue_h26_runtime_activation_once(raw)
        self.assertIs(result, receipt)
        adapter_type.assert_called_once_with(capability, runner.ADMINISTRATIVE_ROOT)
        ...
        for field, replacement in (
            ...
            with self.subTest(field=field), mock.patch.object(runner, "_require_execution_boundary", return_value=capability), mock.patch.object(runner, "_H26BoundaryGatedPosixIssuanceFilesystemAdapter") as adapter_type, self.assertRaises(ValueError):
                runner.issue_h26_runtime_activation_once(raw_bad)
            adapter_type.assert_not_called()

    def test_real_adapter_cannot_be_constructed_without_attested_boundary(self):
        forged_capability = object.__new__(runner._H26ExecutionBoundaryCapability)
        forged_capability.head = "a" * 40
        for value in (object(), forged_capability):
            with self.subTest(value=type(value).__name__), self.assertRaises(PermissionError):
                runner._H26BoundaryGatedPosixIssuanceFilesystemAdapter(
                    value, runner.ADMINISTRATIVE_ROOT
                )
    ...
    def test_real_darwin_adapter_publishes_once_without_replace(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(runner, "_load_entrypoint_contract"), mock.patch.dict(
                os.environ,
                {
                ...
                capability = runner._require_execution_boundary()
                adapter = runner._H26BoundaryGatedPosixIssuanceFilesystemAdapter(
                    capability, str(root)
                )
