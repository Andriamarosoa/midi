...
Moteur causal de transcription **guitare mono propre → MIDI**, utilisable en direct sur Windows, sur un fichier WAV et dans l'application Android fournie.
...
Le produit 1.0 combine le modèle V6.0 `pitch + active + harmoniques` et la porte V6.3.3 de transition entraînée à partir des labels existants `onset`, `note_id` et harmoniques. Les mêmes modèles TFLite et le même contrat de 20 features sont utilisés sur PC et Android.
...
L'APK installable est [GuitarMidiAI-1.0.0-debug.apk](artifacts/guitar_midi_v1_0_0/GuitarMidiAI-1.0.0-debug.apk). Le projet reproductible se trouve dans [android](android/README.md) et compile avec Android SDK 36, JDK 21/17, AGP 9.2.1 et Gradle 9.4.1.

L'application demande le microphone, traite du PCM float 44,1 kHz et utilise automatiquement le premier port MIDI USB/Bluetooth disponible. Sans périphérique MIDI, la note est affichée à l'écran.
...
Le dossier [artifacts/guitar_midi_v1_0_0](artifacts/guitar_midi_v1_0_0) contient les modèles TFLite/ONNX/SavedModel, l'APK, `metadata.json`, `release_manifest.json` et les rapports : parité, latence, stride, backpressure, WAV causal, multisource et harmoniques externes.
...
Les historiques V1→V6 restent documentés dans les README de version. Le produit 1.0 ne prétend pas encore être polyphonique ; cette évolution demande un nouveau contrat de sortie et une validation GAPS dédiée.
