...
- 2026-07-29 — **en cours** — `live_ab_adaptive_attack` : L'audit du train annulé confirme un goulot d'étranglement I/O, pas une erreur CUDA. Le smoke P100 représentatif du snapshot `834b318a` est `COMPLETE` : staging 1 508 fichiers/8 643 963 647 octets en 208,70 s, entraînement-validation 8 192/2 048 en 29,85 s à 274,73 exemples/s, archive 17 745 920 octets et SHA-256 vérifié, `locked_test_used=false`; projection mesurée 2,00 h pour 8 époques avec staging. La reprise exacte est intégrée localement : plans déterministes, A/B compilé avec optimiseur/LR, callbacks sérialisés, transactions idempotentes, reprise intra-époque, fallback corruption, budget et NaN; transport inter-kernels validé par manifeste/taille/SHA/extraction sûre. Le smoke écrit aussi un roundtrip strict dans un processus frais avec versions Keras 3. Les 304 tests locaux passent. Le MCP est bloqué par HTTP 403 `oauthClients.use`; le CLI officiel reste le canal fiable pour statut, logs, outputs et quota. Étape active : commit/push/snapshot, puis smoke P100 multi-chunks et reprise inter-kernels avant tout train complet. Test verrouillé exclu.
