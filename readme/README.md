...
- La seconde revue externe stricte de la chaîne corrigée
  ...
  reste définitivement dormant/false et seule l'activation 5B exacte autorise
  le runtime; binding et seal sont relus byte-for-byte depuis leurs commits et
  comparés au tuple blob/taille/SHA scellé; claim, activation, execution ID et
  nonce sont liés dans les deux runtimes; les corruptions inverses sont injectées
...
- La première revue externe stricte de Review 5A avait rendu `FAIL` avant
  toute science réelle. Son premier correctif fermait sept bloqueurs : invocation
