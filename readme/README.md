...
- Mise à jour : `2026-07-29T04:26:26.1012847+04:00`
- Étape : `dual_stream_bass_candidate`
- Statut : `en cours`
- Détail : Le candidat causal à deux représentations est implémenté au commit source `edc39ff9`. Une entrée de 8192 échantillons alimente le chemin normal 4096 et une petite branche 8192→4096 spécialisée MIDI 40–64. La fusion résiduelle frame/onset est neutre après transfert V2.2. Le candidat ajoute 5 730 paramètres (+1,7 %, 342 996 au total), tient 2,81/2,13/2,31 ms p95 en TFLite à 1/2/3 threads et les tests passent. Le smoke P100 est validé avec transfert V2.2, archive/SHA-256 conformes et `locked_test_used=false`. Le train complet P100 `guitar-midi-dual-bass-train-edc39ff9` reste `RUNNING` ; quota réel consommé 0,80 h sur 30 h. Son snapshot `edc39ff9` ne publie pas le numéro d'époque pendant l'exécution. Pour tous les prochains runs, le pipeline force désormais `PYTHONUNBUFFERED=1`, Keras `verbose=2`, une ligne `EPOCH_PROGRESS` flushée au début et à la fin de chaque époque et un `epoch_progress.json` atomique avec métriques ; 258 tests passent. Cette correction d'observabilité ne modifie pas le train actif. Le rééquilibrage grave et l'objectif fondamentale/résonance restent séparés.
...
- 2026-07-29 — **en cours** — `live_ab_adaptive_attack` : Deux représentations simultanées sont implémentées dans un modèle unique : chemin normal 4096 et petite branche grave compressée 8192→4096, fusion résiduelle MIDI 40–64. Initialisation V2.2 neutre confirmée, +5 730 paramètres (+1,7 %), TFLite p95 sous 2,81 ms. Smoke P100 validé avec archive/SHA-256 conformes et `locked_test_used=false`. Train complet P100 `guitar-midi-dual-bass-train-edc39ff9` actif ; quota GPU réel 0,80 h. Son ancien snapshot ne permet pas de lire l'époque courante. Les prochains runs utilisent sortie non tamponnée, Keras `verbose=2`, lignes `EPOCH_PROGRESS` flushées et `epoch_progress.json` atomique ; 258 tests passent. Sélection et calibration resteront validation-only.
