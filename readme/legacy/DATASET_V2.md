...
```powershell
python -m src.dataset.inspect_dataset --npz "data\processed\stream\00_BN1-129-Eb_comp_hex.npz"
```

## Visualiser un exemple

```powershell
python -m src.dataset.visualize_sample --npz "data\processed\stream\00_BN1-129-Eb_comp_hex.npz" --index 0
```

## Créer les splits

```powershell
python -m src.dataset.split_dataset --manifest "data\processed\stream\manifest.csv"
```
