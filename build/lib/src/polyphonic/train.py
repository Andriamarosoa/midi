...
def _weights(positives: np.ndarray, total: int, maximum: float) -> np.ndarray:
...
def main() -> int:
    ...
    if dataset_fractions:
        frame_positive, natural_frames = dataset_balanced_class_counts(
            train_corpus, "active_bits", dataset_fractions
        )
        onset_positive, _ = dataset_balanced_class_counts(
            train_corpus, "onset_bits", dataset_fractions
        )
    else:
        frame_positive, natural_frames = class_counts(
            train_corpus, "active_bits"
        )
        onset_positive, _ = class_counts(train_corpus, "onset_bits")
    frame_weights = _weights(
        frame_positive, natural_frames, float(training["maximum_frame_weight"])
    )
    onset_weights = _weights(
        onset_positive, natural_frames, float(training["maximum_onset_weight"])
    )
