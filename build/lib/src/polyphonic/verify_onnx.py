...
def verify(artifact_dir: Path) -> dict[str, object]:
    ...
    metrics["frame_decision_agreement"] = float(np.mean(
        (expected["frame"] >= frame_threshold)
        == (raw[mapping["frame"]] >= frame_threshold)
    ))
    metrics["onset_decision_agreement"] = float(np.mean(
        (expected["onset"] >= onset_threshold)
        == (raw[mapping["onset"]] >= onset_threshold)
    ))
    passed = bool(
        metrics["frame_decision_agreement"] == 1.0
        and metrics["onset_decision_agreement"] == 1.0
        and max(
            metrics[name]["maximum_absolute_error"] for name in OUTPUT_NAMES
        ) < 1e-3
    )
