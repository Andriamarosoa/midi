...
def run(args) -> int:
    ...
    activity_gate = CalibratedAudioActivityGate(
        sample_rate,
        hop_samples,
        calibration_s=args.calibration_s,
    )
            ...
            while not stopped:
                ...
                try:
                    ...
                    if overflow != processed_overflow:
                        ...
                        activity_gate.reset_continuity()
                        backpressure = LiveBackpressure(
                ...
                activity_evidence = activity_gate.process_rms(onset_evidence.rms)
                if not (
                    onset_evidence.calibrated and activity_evidence.calibrated
                ):
                    continue
                ...
                if trace is not None:
                    ...
                    trace["onset_rms_threshold_dbfs"].append(
                        CalibratedAudioActivityGate.rms_to_dbfs(
    ...
    if args.report_json:
        ...
        report = {
            ...
            "input_peak_dbfs": CalibratedAudioActivityGate.rms_to_dbfs(
                input_peak_absolute
            ),
            ...
            "audio_activity_gate": activity_gate.diagnostics(),
